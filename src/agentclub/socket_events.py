...
def register_events(socketio):
    ...
    def on_connect(auth_data=None):
        ...
        if agent_token:
            ...
            if not user:
                logger.warning("socket connect rejected: invalid agent_token sid=%s", request.sid)
                return False  # reject connection
            _register_connection(user, request.sid)
            logger.info("socket connect: agent=%s (%s) sid=%s", user["username"], user["id"], request.sid)
            return

        # Web user session auth
        user_id = session.get("user_id")
        if not user_id:
            logger.debug("socket connect rejected: no session sid=%s", request.sid)
            return False
        user = models.get_user_by_id(user_id)
        if not user:
            logger.warning("socket connect rejected: stale session user_id=%s sid=%s", user_id, request.sid)
            return False
        _register_connection(user, request.sid)
        logger.info("socket connect: user=%s (%s) sid=%s", user["username"], user["id"], request.sid)
    ...
    def on_disconnect():
        ...
        if user_id:
            ...
            logger.info("socket disconnect: user=%s sid=%s remaining_sids=%d",
